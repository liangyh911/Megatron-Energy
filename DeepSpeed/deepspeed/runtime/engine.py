...
class DeepSpeedEngine(Module):
    ...
    def allreduce_gradients(self, bucket_size=MEMORY_OPT_ALLREDUCE_SIZE):
        ...
        if self.zero_optimization_partition_gradients():
            if torch.distributed.get_rank() == 0:
                print("self.optimizer.overlapping_partition_gradients_reduce_epilogue()")
    ...
    def backward(self, loss, allreduce_gradients=True, release_loss=False, retain_graph=False, scale_wrt_gas=True):
        ...
        if self.zero_optimization():
            ...
            if self.eigenvalue_enabled():
                ...
                f.write("-----DS back 4\n")
                f.close()
                print("used optimizer: ", self.optimizer)
                self.optimizer.backward(loss, retain_graph=retain_graph)
        elif self.bfloat16_enabled():
    ...
    def _take_model_step(self, lr_kwargs, block_eigenvalue={}):
        if self.gradient_clipping() > 0.0:
            if not (self.fp16_enabled() or self.bfloat16_enabled() or self.amp_enabled() or self.zero_optimization()):
                ...
                # print("self.clip_fp32_gradients()")
            elif self.amp_enabled():
    ...
    def step(self, lr_kwargs=None):
        ...
        if self.is_gradient_accumulation_boundary():
            ...
            # print("is_gradient_accumulation_boundary", self.is_gradient_accumulation_boundary)
    ...
    def allreduce_bucket(self, bucket, dp_group, dp_world_size=None):
        ...
        # print("allreduce_bucket dp_group: ", dp_group)
        ...
        if self.postscale_gradients():
            ...
            monitor.begin_window("ddp_all_reduce_1")
            dist.all_reduce(tensor_to_allreduce, group=dp_group)
            measurement = monitor.end_window("ddp_all_reduce_1")
            # excu_time = time.perf_counter() - start
            
            I = torch.distributed.get_rank()
            f = open("./output/dp_profile/"+str(I)+".txt", 'a')
            # f.write("ddp_all_reduce time: " + str(excu_time) + "\n")
            f.write("Communication: " + str(measurement.total_energy) + "\n") 
            f.close()
            ...
            # start = time.perf_counter()
            monitor.begin_window("ddp_all_reduce_2")
            dist.all_reduce(tensor_to_allreduce, group=dp_group)
            measurement = monitor.end_window("ddp_all_reduce_2")
            # excu_time = time.perf_counter() - start
    ...
    def _reduce_non_expert_gradients(self, grads, elements_per_buffer):
        ...
        # if torch.distributed.get_rank() == 0:
        #     print("split_sparse_tensor_buckets: ", len(split_sparse_tensor_buckets),
        #           "split_dense_tensor_buckets: ", len(split_dense_tensor_buckets))
