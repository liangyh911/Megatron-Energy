...
class PipelineEngine(DeepSpeedEngine):
    ...
    def train_batch(self, data_iter=None):
        ...
        #     print("micro_batches_num: ", self.micro_batches)
        ...
        # if torch.distributed.get_rank() == 0:
        #     print('stage: ', self.stage_id, "time: ", end-start)
    ...
    def _exec_send_activations(self, buffer_id):
        ...
        if isinstance(outputs, torch.Tensor):
            ...
            #     print("activation p2p send time: ", excu_time)
            I = torch.distributed.get_rank()
            f = open("./output/pp_profile/"+str(I)+".txt", 'a')
            f.write("send_activation time: " + str(excu_time) + "\n")
            f.close()
    ...
    def _exec_send_grads(self, buffer_id):
        ...
        if isinstance(inputs, torch.Tensor):
            ...
            excu_time = time.perf_counter() - start
            # if torch.distributed.get_rank() == 1:
            #     print("grads p2p send time: ", excu_time)
            I = torch.distributed.get_rank()
            f = open("./output/pp_profile/"+str(I)+".txt", 'a')
            f.write("send_grads time: " + str(excu_time) + "\n")
            f.close()
        else:
    ...
    def _exec_recv_activations(self, buffer_id):
        ...
        if isinstance(self.pipe_recv_buf, torch.Tensor):
            ...
            excu_time = time.perf_counter() - start
            # if torch.distributed.get_rank() == 1:
            #     print("activation p2p recv time: ", excu_time)
            I = torch.distributed.get_rank()
            f = open("./output/pp_profile/"+str(I)+".txt", 'a')
            f.write("recv_activation time: " + str(excu_time) + "\n")
            f.close()
    ...
    def _exec_recv_grads(self, buffer_id):
        ...
        if isinstance(self.grad_layer, torch.Tensor):
            ...
            excu_time = time.perf_counter() - start
            # if torch.distributed.get_rank() == 1:
            #     print("grads p2p recv time: ", excu_time)
            I = torch.distributed.get_rank()
            f = open("./output/pp_profile/"+str(I)+".txt", 'a')
            f.write("recv_grads time: " + str(excu_time) + "\n")
            f.close()
        else:
    ...
    def _exec_schedule(self, pipe_schedule):
        ...
        self.fwd_outputs = []
        time_map = {
            schedule.OptimizerStep: 0,
            schedule.ReduceGrads: 0,
            schedule.ReduceTiedGrads: 0,
            schedule.LoadMicroBatch: 0,
            schedule.ForwardPass: 0,
            schedule.BackwardPass: 0,
            schedule.SendActivation: 0,
            schedule.RecvActivation: 0,
            schedule.SendGrad: 0,
            schedule.RecvGrad: 0,
        }
        # total_time = 0
        ...
        for step_cmds in pipe_schedule:
            ...
            #     print("stage: ", self.stage_id, "step_cmds: ", step_cmds)
            for cmd in step_cmds:
                # print("step_cmds: ", step_cmds, "cmd: ", cmd)
                ...
                time_map[type(cmd)] = time.perf_counter() - start

                # I = torch.distributed.get_rank()
                # with open("./output/pp_profile/"+str(I)+".txt", 'a') as convert_file:
                #     # convert_file.write(json.dumps(time_map))
                #     convert_file.write(str(time_map))
            
        # print("-------------------End one pipe_schedule-------------------")
        # if torch.distributed.get_rank() == 1:
        #     print("stage: ", torch.distributed.get_rank(), "total exec time: ", time_map)
