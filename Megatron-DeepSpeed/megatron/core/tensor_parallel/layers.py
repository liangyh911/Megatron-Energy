...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, gradient_accumulation_fusion,
        ...
        profileTE.monitor_begin("linearGrad_forward_matmul")
        output = torch.matmul(total_input, weight.t())
        profileTE.monitor_end("linearGrad_forward_matmul", "profile", "Computation")

        if bias is not None:
            profileTE.monitor_begin("linearGrad_forward_add_bias")
            output = output + bias
            profileTE.monitor_end("linearGrad_forward_add_bias", "profile", "Computation")
        return output
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.sequence_parallel:
            ...
            total_input = input
        profileTE.monitor_begin("linearGrad_backward_matmul")
        grad_input = grad_output.matmul(weight)
        profileTE.monitor_end("linearGrad_backward_matmul", "profile", "Computation")
        ...
        if ctx.async_grad_allreduce:
            profileTE.monitor_begin("linearGrad_backward_all_reduce")
            # Asynchronous all-reduce
            handle = torch.distributed.all_reduce(
                    grad_input, group=get_tensor_model_parallel_group(), async_op=True)
            # Here we rely on CUDA_DEVICE_MAX_CONNECTIONS=1 to ensure that the
            # all-reduce is scheduled before the weight gradient computation
            profileTE.monitor_end("linearGrad_backward_all_reduce", "profile", "Communication")
        ...
        if ctx.async_grad_allreduce:
            profileTE.monitor_begin("linearGrad_backward_wait")
            handle.wait()
            profileTE.monitor_end("linearGrad_backward_wait", "profile", "Communication")
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def forward(self,
        ...
        if self.gather_output and not self.is_expert_without_slicing:
            ...
            profileTE.monitor_begin("col_para_all_gather")
            output = gather_from_tensor_model_parallel_region(output_parallel)
            profileTE.monitor_end("col_para_all_gather", "profile", "Communication")
...
class RowParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_):
        ...
        if self.input_is_parallel or self.is_expert_without_slicing:
            ...
            assert not self.sequence_parallel
            profileTE.monitor_begin("row_para_backprop_all_reduce")
            input_parallel = scatter_to_tensor_model_parallel_region(input_)
            profileTE.monitor_end("row_para_backprop_all_reduce", "profile", "Communication")
        # Matrix multiply.
        output_parallel = linear_with_grad_accumulation_and_async_allreduce(
            input=input_parallel,
            weight=self.weight,
        ...
        if self.sequence_parallel:
            ...
            profileTE.monitor_begin("row_para_all_reduce")
            output_ = reduce_from_tensor_model_parallel_region(output_parallel)
            profileTE.monitor_end("row_para_all_reduce", "profile", "Communication")
        if not self.skip_bias_add:
            if self.bias is not None:
                profileTE.monitor_begin("row_para_add_bias")
                output = output_ + self.bias
                profileTE.monitor_end("row_para_add_bias", "profile", "Computation")
            else:
                output = output_
