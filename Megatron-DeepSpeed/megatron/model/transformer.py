...
from megatron import profileTE
...
class ParallelMLP(MegatronModule):
    ...
    def forward(self, hidden_states):
        ...
        profileTE.monitor_begin("MLP_activation")
        ...
        profileTE.monitor_end("MLP_activation", "profile", "Computation")
...
class CoreAttention(MegatronModule):
    ...
    def forward(self, query_layer, key_layer,
        ...
        profileTE.monitor_begin("QK_matmul")
        matmul_result = torch.baddbmm(
            matmul_input_buffer,
            query_layer.transpose(0, 1),   # [b * np, sq, hn]
            key_layer.transpose(0, 1).transpose(1, 2),  # [b * np, hn, sk]
            beta=0.0, alpha=(1.0/self.norm_factor))
        profileTE.monitor_end("QK_matmul", "profile", "Computation")
        ...
        profileTE.monitor_begin("attn_mask_softmax")
        attention_probs = self.scale_mask_softmax(attention_scores,
                                                  attention_mask)
        profileTE.monitor_end("attn_mask_softmax", "profile", "Computation")

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        profileTE.monitor_begin("attn_dropout")
        if not self.sequence_parallel:
            with tensor_parallel.get_cuda_rng_tracker().fork():
                attention_probs = self.attention_dropout(attention_probs)
        else:    
            attention_probs = self.attention_dropout(attention_probs)
        profileTE.monitor_end("attn_dropout", "profile", "Computation")
        ...
        # matmul: [b * np, sq, hn]
        profileTE.monitor_begin("attn_context_layer")
        context_layer = torch.bmm(attention_probs, value_layer.transpose(0, 1))
        profileTE.monitor_end("attn_context_layer", "profile", "Computation")
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask=None,
        ...
        if self.drop_path is None:
            ...
            if attention_bias is not None:
                attention_bias = attention_bias.expand_as(residual)
            profileTE.monitor_begin("res_connect_drop_path")
            with self.bias_dropout_add_exec_handler():
                layernorm_input = bias_dropout_add_func(
                    attention_output,
                    attention_bias,
                    residual,
                    self.hidden_dropout)
            profileTE.monitor_end("res_connect_drop_path", "profile", "Computation")
        else:
            profileTE.monitor_begin("res_connect_no_drop_path")
            out = torch.nn.functional.dropout(attention_output + attention_bias,
                                              p=self.hidden_dropout,
                                              training=self.training)
            layernorm_input = residual + self.drop_path(out)
            profileTE.monitor_end("res_connect_no_drop_path", "profile", "Computation")

        # Layer norm post the self attention.
        profileTE.monitor_begin("post_attention_layernorm")
        layernorm_output = self.post_attention_layernorm(layernorm_input)
        profileTE.monitor_end("post_attention_layernorm", "profile", "Computation")
        ...
        if self.drop_path is None:
            ...
            profileTE.monitor_begin("second_res_connect_drop_path")
            with self.bias_dropout_add_exec_handler():
                output = bias_dropout_add_func(
                    ...
                    residual,
                    self.hidden_dropout)
            profileTE.monitor_end("second_res_connect_drop_path", "profile", "Computation")
            ...
            profileTE.monitor_begin("second_res_connect_no_drop_path")
            if mlp_bias is not None:
            ...
            profileTE.monitor_end("second_res_connect_no_drop_path", "profile", "Computation")
