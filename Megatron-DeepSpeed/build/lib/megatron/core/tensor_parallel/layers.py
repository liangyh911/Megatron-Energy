...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, gradient_accumulation_fusion,
        ...
        # excu_time = time.perf_counter() - start
        # if torch.distributed.get_rank() == 0:
        #     print("linear_with_grad_accumulation_and_async_allreduce perpare time: ", excu_time)
        # I = torch.distributed.get_rank()
        # f = open("./output/profile/"+str(I)+".txt", 'a')
        # f.write("linear_with_grad_accumulation_and_async_allreduce perpare time: " + str(excu_time) + "\n")
        # f.close()
        ...
        # excu_time = time.perf_counter() - start
        # if torch.distributed.get_rank() == 0:
        #     print("linear_with_grad_accumulation_and_async_allreduce matmul time: ", excu_time)
        # I = torch.distributed.get_rank()
        # f = open("./output/profile/"+str(I)+".txt", 'a')
        # f.write("linear_with_grad_accumulation_and_async_allreduce matmul time: " + str(excu_time) + "\n")
        # f.close()
        ...
        if bias is not None:
            ...
            # excu_time = time.perf_counter() - start
            # if torch.distributed.get_rank() == 0:
            #     print("linear_with_grad_accumulation_and_async_allreduce add bias: ", excu_time)
            # I = torch.distributed.get_rank()
            # f = open("./output/profile/"+str(I)+".txt", 'a')
            # f.write("linear_with_grad_accumulation_and_async_allreduce add bias: " + str(excu_time) + "\n")
            # f.close()
        return output
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, *,
        ...
        # if torch.distributed.get_rank() == 0: 
        #     print("world_size", world_size, "output_size_per_partition: ", self.output_size_per_partition, 
        #             "input_size", self.input_size, "weights size: ", self.weight.size())
    ...
    def forward(self,
        ...
        # if torch.distributed.get_rank() == 0: 
        #     print("input size: ", input_parallel.size(), "weight size: ", weight.size(), "output size: ", output_parallel.size())
        ...
        if self.gather_output and not self.is_expert_without_slicing:
            ...
            # excu_time = time.perf_counter - start
            # if torch.distributed.get_rank() == 0:
            #     print("-col_linear_all_gather time: ", excu_time)
            
            # I = torch.distributed.get_rank()
            # f = open("./output/profile/"+str(I)+".txt", 'a')
            # f.write("-col_linear_all_gather time: " + str(excu_time) + "\n")
            # f.close()
        else:
...
class RowParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_):
        ...
        # excu_time = time.perf_counter() - start
        # if torch.distributed.get_rank() == 0:
        #     print("-row_linear_backprop_all_reduce time: ", excu_time)
        
        # I = torch.distributed.get_rank()
        # f = open("./output/profile/"+str(I)+".txt", 'a')
        # f.write("-row_linear_backprop_all_reduce time: " + str(excu_time) + "\n")
        # f.close()
        ...
        # excu_time = time.perf_counter() - start
        # if torch.distributed.get_rank() == 0:
        #     print("-Matrix multiply time: ", excu_time)
        
        # I = torch.distributed.get_rank()
        # f = open("./output/profile/"+str(I)+".txt", 'a')
        # f.write("-Matrix multiply time: " + str(excu_time) + "\n")
        # f.close()
        ...
        if self.sequence_parallel:
            ...
            # excu_time = time.perf_counter() - start
            # if torch.distributed.get_rank() == 0:
            #     print("-row_linear_all_reduce time: ", excu_time)
            
            # I = torch.distributed.get_rank()
            # f = open("./output/profile/"+str(I)+".txt", 'a')
            # f.write("-row_linear_all_reduce time: " + str(excu_time) + "\n")
            # f.close()
        ...
        if not self.skip_bias_add:
            ...
            # excu_time = time.perf_counter() - start
            # if torch.distributed.get_rank() == 0:
            #     print("-row_linear_add_bias time: ", excu_time)
            
            # I = torch.distributed.get_rank()
            # f = open("./output/profile/"+str(I)+".txt", 'a')
            # f.write("-row_linear_add_bias time: " + str(excu_time) + "\n")
            # f.close()
