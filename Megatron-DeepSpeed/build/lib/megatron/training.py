...
def pretrain(train_valid_test_dataset_provider,
    ...
    # print("train: ",  1 if train_data_iterator == None else 0,\
    #         "test: ", 1 if test_data_iterator == None else 0)
            # "valid: ", 1 if valid_data_iterator == None else 0)
...
def setup_model_and_optimizer(model_provider_func,
    ...
    # print("if deepspeed engine: ", isinstance(model[0], deepspeed.PipelineEngine))
...
def train_step(forward_step_func, data_iterator,
    ...
    # if mpu.get_tensor_model_parallel_rank() == 0:
    #     print("number of microbatch: ", get_num_microbatches())
    ...
    if args.deepspeed:
        # print("deepspeed update parameters")
        increment = get_num_microbatches() * \
                    args.micro_batch_size * \
                    args.data_parallel_size
        # print("call setp function")
        model[0].step(lr_kwargs={'increment': increment})
        update_successful = model[0].was_step_applied()
        # print("model is DeepSpeedEngine? ", isinstance(model[0], deepspeed.DeepSpeedEngine))
    else:
        # print("no deepspeed update parameters")
        update_successful, grad_norm, num_zeros_in_grad = optimizer.step(args, timers)
    timers('optimizer').stop()

    # Gather params.
    if not args.deepspeed and update_successful:
        # print("no deepspeed Gather params")
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    args.train_iters = 1
    # print("train_iters: ", args.train_iters, "iteration: ", iteration)
...
def build_train_valid_test_data_loaders(
    ...
    if rank_in_parallel_group == 0:
        ...
        # print("train: ",  1 if train_dataloader == None else 0,\
        #     "test: ", 1 if test_dataloader == None else 0)
    else:
